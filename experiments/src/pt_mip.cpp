...
void lifted_122(const Point *A, iter_t start, iter_t end, bool *r, double &px) {
    double _min_x = px;
    for(iter_t i = start; i < end; i++) {
        _min_x = min(A[i].x, _min_x);
        bool is_opt = true;
        for (iter_t j = start; j < end; j++) {
            if(i != j) {
...
void join_lifted_122(Point *A, iter_t start, iter_t end, bool *r, double min_x) {
    for(iter_t i = start; i < end; i++) {
        if(r[i] && A[i].x > min_x) {
            r[i] = false;
        }
...
void _soln_122(int r, Point *A, iter_t start, iter_t end, bool *res, double &px) {
    if(end - start <= PARTITION_MIN || r >= RECURSION_MAX) {
        lifted_122(A, start, end, res, px);
    } else {
        iter_t mid = 0;
        double px_right = 0.;
        double px_left = 0.;
        ...
        task_group g;
        g.run([&] { _soln_122(r+1, A, start, mid, res, px_left); });
        g.run([&] { _soln_122(r+1, A, mid, end, res, px_right); });
        g.wait();
        join_lifted_122(A, mid, end, res, px_left);
        px = max(px_left, px_right);
    }
}
...
void _seq_soln_122(int r, Point *A, iter_t start, iter_t end, bool *res, double &px) {
    if(end - start <= PARTITION_MIN || r >= RECURSION_MAX) {
        lifted_122(A, start, end, res, px);
    } else {
        iter_t mid = 0;
        ...
        double min_right = 0;
        partition_122(A, start, end, mid);
        _seq_soln_122(r+1, A, start, mid, res, min_left);
        _seq_soln_122(r+1, A, mid, end, res, min_right);
        join_lifted_122(A, mid, end, res, min_left);
        px = min(min_left, min_right);
    }
}
...
//  Pivot2 selection
    double pivot2 = 0.;
    for(iter_t j = start; j < end; j++){
        pivot2 = A[j].y <= pivot1 ? min(pivot2, A[j].x) : pivot2;
    }
...
void join_100(const Point *A, bool* res, const iter_t start, const iter_t end, const double min_x) {
    for(iter_t i = start; i < end; i++){
        if(res[i] && A[i].x > min_x)  res[i] = false;
    }
}
...
void _soln_100(int r, const Point *A, iter_t start, iter_t end, bool *res, double &px){
    if(end - start < PARTITION_MIN || r > RECURSION_MAX) {
        lifted_122(A, start, end, res, px);
    } else {
        iter_t mid = (end + start) / 2;
        ...
        double px_left = 0;
        task_group g;
        g.run( [&]{_soln_100(r+1, A, start, mid, res, px_left);});
        g.run( [&]{_soln_100(r+1, A, mid, end, res, px_right);});
        g.wait();
        join_100(A, res, start, mid, px_right);
        px = min(px_left, px_right);
    }
}
...
void soln_100(int r, Point *A, iter_t start, iter_t end, bool *res){
    ...
    double px = 0;
    _soln_100(0, A, start, end, res, px);
}




void _seq_soln_100(int r, const Point *A, iter_t start, iter_t end, bool *res, double &min_x){
    if(end - start < PARTITION_MIN || r > RECURSION_MAX) {
        lifted_122(A, start, end, res, min_x);
    } else {
        iter_t mid = (end + start) / 2;
        double min_right = 0;
        double min_left = 0;
        _seq_soln_100(r+1, A, start, mid, res, min_left);
        _seq_soln_100(r+1, A, mid, end, res, min_right);
        join_100(A, res, start, mid, min_right);
        min_x = max(min_left, min_right);
    }
}
