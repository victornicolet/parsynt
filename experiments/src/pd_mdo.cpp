...
#define EXPERIMENT_NAME "mdo"

using namespace std;
using namespace tbb;


struct ex_struct { bool a; int cd; int md; };

struct ex_struct_lifted { bool a; int cd; int md; int bd;};
...
void ex_lifted_join(ex_struct_lifted &lhs, ex_struct_lifted rhs) {
    lhs.cd = rhs.cd;
    lhs.md = max(max(rhs.md, lhs.md), lhs.cd + rhs.bd);
    lhs.bd = lhs.bd + (!rhs.a ? rhs.bd : 0);
}
...
struct EX_LIFTED {
    ...
    explicit EX_LIFTED(int* _A, long sz) : res((ex_struct_lifted){true, 0, 0 ,0 }), A(_A) {}
...
struct EX_PREFIX {
    ...
    explicit EX_PREFIX(int* _A, long sz) : res((ex_struct){true, 0, 0}), A(_A), ps(-1), pe(-1) {}
