...
//   Pivot1 selection
    ...
    Point pivot1 = A[start + end / 2];

    double pivot2 = 0.;
    for(iter_t j = start; j < end; j++){
        pivot2 = A[j].y <= pivot1.y ? max(pivot2, A[j].x) : pivot2;
    }
...
auto main(int argc, char** argv) -> int {
    // Data size:
    if(argc < 2) {
        cout << "Usage:./PtExpQoc NUM_POINTS [MAX_THREADS] [OPT_RATIO] [EXPCODE]" << endl;
