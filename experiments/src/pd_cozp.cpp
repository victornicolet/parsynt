...
struct ex_struct_lifted { bool a; bool b; bool c; int count; };
...
void ex_lifted_join(ex_struct_lifted &lhs, ex_struct_lifted rhs) {
    ...
    lhs.b = rhs.b;
    lhs.c = lhs.c;
    lhs.count = lhs.count + rhs.count + (lhs.a && rhs.c ? 1 : 0);
}
...
struct EX_LIFTED {
    ...
    explicit EX_LIFTED(int* _A, long sz) : res((ex_struct_lifted){false,false,false,0}), A(_A) {}
...
int main(int argc, char** argv) {
    // Data size:
    if(argc < 2) {
        cout << "Usage:./ExpCozp SIZE [MAX_CORES]" << endl;
        return  -1;
    }
