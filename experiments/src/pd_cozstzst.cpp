...
#define EXPERIMENT_NAME "cozstzst"

using namespace std;
using namespace tbb;


struct ex_struct { bool a; bool b; bool c; bool d; int count; };

struct ex_struct_lifted {
    bool a; bool b; bool c; bool d; int count;
    bool b0; bool c0; bool d0; int cnt0;
    bool c1; bool d1; int cnt1;
    bool d2; int cnt2;
};
...
void seq_lifted(int *A, long start, long end, ex_struct_lifted &x) {
    bool a = x.a; bool b = x.b; bool c = x.c; bool d = x.d; int count  = x.count;
    bool b0 = x.b0; bool c0 = x.c0; bool d0 = x.d0; int cnt0 = x.cnt0;
    bool c1 = x.c1; bool d1 = x.d1; int cnt1 = x.cnt1;
    bool d2 = x.d2; int cnt2 = x.cnt2;
    ...
    for(long i = start; i < end; i++) {
        ...
        cnt0 = elt == 3 && (c0 || d0) ? 1 : cnt0;
        d0 = elt == 0 && (c0 || d0);
        c0 = elt == 2 && b0;
        b0 = elt == 0 && b0;

        cnt1 = elt == 3 && (c1 || d1) ? 1 : cnt1;
        d1 = elt == 0 && (c1 || d1);
        c1 = elt == 2 && !c1;

        cnt2 = elt == 3 && d2 ? 1 : cnt2;
        d2 = elt == 0 && d2;
    }
    x.b = b; x.a = a; x.d = d; x.count = count;

    x.b0 = b0; x.c0 = c0; x.d0 = d0; x.cnt0 = cnt0;

    x.c1 = c1; x.d1 = d1; x.cnt1 = cnt1;
...
void ex_lifted_join(ex_struct_lifted &lhs, ex_struct_lifted rhs) {
    ...
    lhs.d = rhs.d;
    lhs.count = lhs.count + rhs.count +
            (lhs.a || lhs.b ? rhs.cnt0 : 0) +
            (lhs.b || lhs.a ? rhs.cnt1 : 0) +
            (lhs.c || lhs.d ? rhs.cnt2 : 0);
}
...
struct EX_LIFTED {
    ...
    explicit EX_LIFTED(int* _A, long sz) :
    res((ex_struct_lifted){false,false, false, true, 0,
                           true, false, false, 0,
                           false, false, 0,
                           true, 0}),
    A(_A) {}
...
struct EX_PREFIX {
    ...
    explicit EX_PREFIX(int* _A, long sz) : res((ex_struct){false,false, false, false, 0}), A(_A), ps(-1), pe(-1) {}
...
double do_seq(int *A, long n) {
    ex_struct res = (ex_struct) {false,false, false, false, 0};
    seq_implem(A, 0, n, res);

    StopWatch t;
    double elapsed = 0.0;
    for(int i = 0; i < NUM_REPEAT; i++) {
        res = (ex_struct) {false, false, false, false, 0};
        t.start();
        seq_implem(A, 0, n, res);
        elapsed += t.stop();
    }
...
bool incorrect(int* A, long n) {
    ex_struct r_seq = (ex_struct) { false, false, false, false,0};
...
int main(int argc, char** argv) {
    // Data size:
    if(argc < 2) {
        cout << "Usage:./ExpCozst SIZE [MAX_CORES]" << endl;
        return  -1;
    }
