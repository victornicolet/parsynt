...
bool seq_implem(Point *A, iter_t start, iter_t end) {
    bool inters = false;

    for(long i = start; i < end; i++) {
        Point p = A[i];
        for(long j = i; j < end; j++) {
            inters = inters || intersect(p, A[j]);
        }
    }
...
ex_lifted join_022(ex_lifted left, ex_lifted right) {
    bool b = left.b || right.b || intersect(left.r, right.l);
...
void seq_soln033(int r, Point *A, iter_t start, iter_t end, bool &b) {
...
int main(int argc, char** argv) {
    // Data size:
    if(argc < 2) {
        cout << "Usage:./PtExpIns NUM_SEGS [NUM_THREADS] [INTER_RATIO] [EXPCODE]" << endl;
        cout << "                 NUM_SEGS : number of segments in the input." << endl;
        cout << "     optional INTER_RATIO : ratio of intersecting segments in input." << endl;
        cout << "         optional EXPCODE : string of two zeroes and ones" << endl;
        cout << "                            #1 : B = (0,2,2)" << endl;
        cout << "                            #2 : B = (0,3,3)" << endl;
        return  -1;
    }
    ...
    auto num_inters = (int) floor(ratio * n);
    // Experiment code
