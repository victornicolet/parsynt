...
let is_struct_assgn (id, e) =
  ...
  | Apply_e (f, el) -> (
      ...
      | [ hd ] -> (
          match hd with
          | Id_e id -> str_begins_with lstate_name id || str_begins_with rstate_name id
          | _ -> false)
      | _ -> false)
  | _ -> false
...
let rec rem_struct_assigns (e : RAst.expr) =
  match e with
  | Let_e (assgn_list, e') | Letrec_e (assgn_list, e') ->
      let filtered_assgns = L.filter (fun a -> not (is_struct_assgn a)) assgn_list in
...
let get_solved_sketch_info (el : RAst.expr list) =
  let join_body =
    try
      match List.find identify_join_func el with Def_e (_, jbody) -> jbody | _ -> raise Not_found
    with Not_found ->
      eprintf "Couldn't find a join in the solution... Did we really find a solution ?@.";
      eprintf "Received: %a@." RAst.pp_expr_list el;
      raise Not_found
  in
  let init_state = Lt.some_hd (L.map check_option (L.filter is_some (L.map get_values_init el))) in
  let identity_state =
