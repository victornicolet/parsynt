...
let get_inloop_info vars : fnExpr -> VarSet.t * VarSet.t =
  ...
  let _join s1 s2 = (VarSet.union (fst s1) (fst s2), VarSet.union (snd s1) (snd s2)) in
...
let rec restrict_func (old_ctx : context) (variables : VarSet.t) (func : fnExpr) : fnExpr =
  let update_rectype name stl =
    ...
    let name' =
      let vs' =
        VarSet.filter
          (fun var -> List.exists (fun (s, t) -> s = var.vname && t = var.vtype) stl')
          variables
      in
      record_name vs'
    in
    match stl' with [] -> Bottom | _ -> Record (name', stl')
  in
  let cases e = match e with FnLetIn _ | FnRec _ | FnRecord _ -> true | _ -> false in
    ...
    | FnVariable var -> (
        ...
        | Record (name, stl) ->
            ...
            if new_rtype = var.vtype then
              ...
              Some (var, var)
            else Some (var, mkFnVar "x_" new_rtype)
        (* No restriction. *)
        | _ -> None)
    | _ -> None
  in
    ...
    List.map
      ...
      (List.filter
         (fun (v, e) ->
           ...
           match var.vtype with
           | Record (name, stl) -> (
               match update_rectype name stl with Bottom -> false | _ -> true)
           | _ -> VarSet.mem (var_of_fnvar v) variables)
         bds)
  in
  ...
  let restrict_body f e =
    ...
    | FnLetIn (bindings, expr) -> (
        match restrict_bindings f bindings with
        | [] -> f expr
        | l ->
            let substs, bindings = Base.List.unzip l in
...
let get_dependent_subsets (problem : prob_rep) : VarSet.t list =
  let dep_map =
    let f k d = VarSet.inter problem.scontext.state_vars (VarSet.filter (fun v -> v.vid != k) d) in
    IM.mapi f (collect_dependencies problem.scontext problem.main_loop_body)
  in
  if !verbose then (
    printf "[INFO] Dependencies:@.";
    IM.iter
      (fun k deps ->
        printf "    %s <-- %a@." (VarSet.find_by_id problem.scontext.all_vars k).vname
          VarSet.pp_var_names deps)
      dep_map);
  rank_and_cluster problem.scontext.state_vars dep_map

let get_increments (problem : prob_rep) : prob_rep list =
  let pb = InnerFuncs.inline_inner ~inline_pick_join:true (Dimensions.width ()) problem in
  ...
  if !verbose then (
    ...
    List.iteri
      (fun i varset -> Format.printf "@[<v 4>    %i : %a@]@." i VarSet.pp_vs varset)
      subsets);
...
let complete_scalar_sketch (sketch : fnExpr) (solution : fnExpr) : fnExpr =
  let rec _c h flat_c =
    ...
    match h with
    | FnLetIn (hl, he) -> FnLetIn (_cb hl flat_c, _c he flat_c)
    | FnRecord (hvs, hemap) -> wrap_state (_cb (unwrap_state hvs hemap) flat_c)
    | _ -> h
  in
  ...
  let rec _c h c =
    ...
    | FnLetIn (bsk, FnLetIn (bsk2, esk)), FnLetIn (bsol, esol) ->
        let common1, common2 =
          ...
          (VarSet.cardinal (VarSet.inter a1 b), VarSet.cardinal (VarSet.inter a2 b))
        in
...
let remove_from_loop_state (variables : VarSet.t) (sketch : fnExpr) : fnExpr =
  ...
  let change_loopres_binder v =
    match v with
    | FnVariable var when var = loopres_binder -> FnVariable new_loopres_binder
    | _ -> v
  in
  ...
  let transform_loop_body in_binder body =
    let case e =
      match e with FnLetIn _ -> true | FnRecord _ -> true | FnRec _ -> true | _ -> false
    in
    let on_case f e =
      match e with
      | FnLetIn (bindings, expr) ->
    ...
    in
    let on_var v =
      match v with FnVariable var when var = in_binder -> FnVariable new_inner_binder | _ -> v
    in
    transform_expr2 { case; on_case; on_var; on_const = identity } body
  in

  let transform_initial_state s init_expr = transform_loop_body s init_expr in
      ...
      on_case =
        (fun f e ->
          ...
          | FnRec (igu, (vs, bs), (s, body)) ->
              FnRec
                ( igu,
                  (VarSet.diff vs variables, transform_initial_state s bs),
                ...
                ( vs,
                  IM.mapi
                    (fun k e ->
                      ...
                      with Not_found -> change_binders e)
                    emap )
          | FnLetIn (blist, expr) ->
              FnLetIn (List.map (fun (v, e) -> (change_loopres_binder v, f e)) blist, f expr)
...
let compose_loop (sol : prob_rep) (prev_solution : fnExpr) (sketch : fnExpr) : fnExpr =
  ...
  let core =
    ...
    | _ -> failhere __FILE__ "compose_loop" "Bad top form."
  in
  match scalars with [] -> core | _ -> FnLetIn (scalars, core)

let complete_increment ~(inner : bool) (increment : prob_rep) (sol : prob_rep option) : prob_rep =
  match sol with
  | Some sol ->
      let prev_solution = if inner then sol.memless_solution else sol.join_solution in
      let incr_sketch = if inner then increment.memless_sketch else increment.join_sketch in
      let new_sketch =
        if has_loop incr_sketch then
          if has_loop prev_solution then (
            (* Match 'one on one' *)
            if !verbose then printf "[INFO] Loop solution, loop sketch.@.";
            compose_loop sol prev_solution incr_sketch)
          else (
            ...
            *)
            ...
            compose prev_solution (remove_from_loop_state sol.scontext.state_vars incr_sketch))
        else (
