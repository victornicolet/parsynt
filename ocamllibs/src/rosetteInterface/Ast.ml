...
let rec pp_expr fmt e =
  ...
  | Float_e f -> pp_print_float fmt f
...
let transform (t : ast_transformer) (e : expr) : expr =
  let rec  aux_t e =
    ...
    | None ->
      ...
      (* Functional forms *)
      | Let_e (se_list, e) ->
        Let_e (List.map (fun (s, e) -> (s, aux_t e)) se_list, aux_t e)
      | Letrec_e (se_list, e) ->
        Letrec_e (List.map (fun (s, e) -> (s, aux_t e)) se_list, aux_t e)
  ...
  try
    begin
      try
        Int_e
          (match op with
           | Plus -> i1 + i2 | Minus -> i1 - i2 | Mul -> i1 * i2
           | Div -> i1 / i2 | Mod ->  i1 mod i2
           | Max -> max i1 i2 | Min -> min i1 i2
           | _ -> exn_arith ())
      with
      | AstArith ->
        Bool_e
          (match op with
           | Eq -> i1 = i2 | Neq -> i1 != i2 | Gt -> i1 > i2
           | Lt -> i1 < i2 | Geq -> i1 >= i2 | Leq -> i1 <= i2
           | _ -> exn_comp ())
    end
  with
  | AstCompar | AstArith -> exn_eval "Bad operator for two integers."
  ...
  try
    begin
      try
        Float_e
          (match op with
           | Plus -> f1 +. f2 | Minus -> f1 -. f2 | Mul -> f1 *. f2
           | Div -> f1 /. f2
           | Max -> max f1 f2 | Min -> min f1 f2
           | _ -> exn_arith ())
      with
      | AstArith ->
        Bool_e
          (match op with
           | Eq -> f1 = f2 | Neq -> f1 != f2 | Gt -> f1 > f2
           | Lt -> f1 < f2 | Geq -> f1 >= f2 | Leq -> f1 <= f2
           | _ -> exn_comp ())
    end
  with
  | AstCompar | AstArith -> exn_eval "Bad operator for two floats."

let apply_bool op b1 b2 =
  try
    Bool_e
      (match op with
       | And -> b1 && b2 | Or -> b1 || b2 | Eq -> b1 = b2 | Neq -> b1 != b2
       | _ -> exn_logic ())
  with
  | AstLogic -> exn_eval "Bad operator for two bools."
