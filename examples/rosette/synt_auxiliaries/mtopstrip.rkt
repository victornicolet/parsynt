...
(struct $i (strip_sum) #:transparent)
(define ($i-eq? s1 s2)
  (and (eq? ($i-strip_sum s1) ($i-strip_sum s2))
       ))
...
(define-symbolic
  ...
  seq:strip_sum$6
  seq:strip_sum$7
  seq:strip_sum$8 integer?)

(define seq:strip_sum
  (list seq:strip_sum$0
        seq:strip_sum$1
        seq:strip_sum$2
        seq:strip_sum$3
        seq:strip_sum$4
...
(struct $ii (strip_sum max_top_strip) #:transparent)

(define ($ii-eq? s1 s2)
  (and (eq? ($ii-strip_sum s1) ($ii-strip_sum s2))
...
(define (join#L__mto#6 $L $R j_start j_end)
  (let ([l.strip_sum ($i-strip_sum $L)]
        [r.strip_sum ($i-strip_sum $R)])
    ($i (+ l.strip_sum (+ r.strip_sum 0)))))
...
(define (__loop_body__ s i_begin_ i_end_ )
  (Loop i_begin_ i_end_ 9 s
        (lambda (__s1 i)
          (let ([strip_sum ($ii-strip_sum __s1)]
                [max_top_strip ($ii-max_top_strip __s1)])
            (let ([tup$ ($i (+ strip_sum
                               (+ (list-ref seq:strip_sum i) 0)))])
              (let ([strip_sum ($i-strip_sum tup$)])
                ($ii strip_sum (max max_top_strip strip_sum))))))))

;;Wrapping for the sketch of the join.
(define (__join__ left_state2 right_state3 i_start i_end)
  (let ([l.strip_sum ($ii-strip_sum left_state2)]
        [l.max_top_strip ($ii-max_top_strip left_state2)])
    (let ([r.strip_sum ($ii-strip_sum right_state3)]
