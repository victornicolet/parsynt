...
{
  DfMin(DfMax(DfMin(leftM, rightM2), rightM), leftM2)
}

function MJoin(leftM2 : int, leftM : int, rightM2 : int, rightM : int): int
{
  DfMin((rightM2 + 1), DfMin(leftM, rightM))
}
...
lemma BaseCaseM(a : seq<int>)
  ensures M(a) == MJoin(M2(a), M(a), M2([]), M([]))
  {}

lemma HomM(a : seq<int>, R_a : seq<int>)
  ensures M(a + R_a) == MJoin(M2(a), M(a), M2(R_a), M(R_a))
  {
    if R_a == [] 
    {
    assert(a + [] == a);
