...
function MtsJoin(leftMts : int, leftAux_1 : int, rightMts : int, rightAux_1 : int): int
{
  DfMax((leftMts + rightAux_1), rightMts)
}
...
lemma BaseCaseMts(a : seq<int>)
  ensures Mts(a) == MtsJoin(Mts(a), Aux_1(a), Mts([]), Aux_1([]))
  {}

lemma HomMts(a : seq<int>, R_a : seq<int>)
  ensures Mts(a + R_a) == MtsJoin(Mts(a), Aux_1(a), Mts(R_a), Aux_1(R_a))
  {
    if R_a == [] 
    {
    assert(a + [] == a);
    ...
    MtsJoin(Mts(a), Aux_1(a), Mts(R_a), Aux_1(R_a));
    } // End calc.
  } // End else.
} // End lemma.
